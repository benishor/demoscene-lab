...
namespace Acidrain {
    ...
    TextureGenerator& TextureGenerator::whiteNoise(unsigned char layer) {
        uint32_t* input = reinterpret_cast<uint32_t*>(layers[layer]);
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                *input++ = rand();
    ...
    TextureGenerator& TextureGenerator::brick(unsigned char layer,
                                              unsigned char brickWidth,
                                              unsigned char brickHeight,
        ...
        uint32_t mortar = asInt(mortarColour);
        uint32_t brick = asInt(brickColour);
        ...
        y = gapSize;
        while (y < height) {
            int x = isOddRow ? 0 : brickWidth / 2;
            while (x < width) {
                for (int i = 0; i < gapSize; i++) {
                    for (int currentBrickY = 0; currentBrickY < brickHeight; currentBrickY++) {
                        input[x + i + (currentBrickY + y) * width] = mortar;
                    }
                }
